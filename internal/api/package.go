...
func (server *Server) createPackage(ctx *gin.Context) {
	...
	packageParams := db2.CreatePackageParams{
		TagID:                             req.TagID,
...
func (server *Server) updatePackage(ctx *gin.Context) {
	...
	productPackage, err := server.store.UpdatePackage(ctx, db2.UpdatePackageParams{
		ID:                                req.ID,
		TagID:                             req.TagID,
		PackageType:                       req.PackageType.String,
		Quantity:                          req.Quantity.Decimal,
		Notes:                             req.Notes.String,
		PackagedDateTime:                  req.PackagedDateTime.Time,
		HarvestDateTime:                   req.HarvestDateTime,
		LabTestingState:                   req.LabTestingState.String,
		LabTestingStateDateTime:           req.LabTestingStateDateTime,
		IsTradeSample:                     req.IsTradeSample.Bool,
		IsTestingSample:                   req.IsTestingSample.Bool,
		ProductRequiresRemediation:        req.ProductRequiresRemediation.Bool,
		ContainsRemediatedProduct:         req.ContainsRemediatedProduct.Bool,
		RemediationDateTime:               req.RemediationDateTime,
		ReceivedDateTime:                  req.ReceivedDateTime,
		ReceivedFromManifestNumber:        req.ReceivedFromManifestNumber,
		ReceivedFromFacilityLicenseNumber: req.ReceivedFromFacilityLicenseNumber,
		ReceivedFromFacilityName:          req.ReceivedFromFacilityName,
		IsOnHold:                          req.IsOnHold.Bool,
		ArchivedDate:                      req.ArchivedDate,
		FinishedDate:                      req.FinishedDate,
		ItemID:                            req.ItemID,
		ProvisionalLabel:                  req.ProvisionalLabel,
		IsProvisional:                     req.IsProvisional.Bool,
		IsSold:                            req.IsSold.Bool,
		PpuDefault:                        req.PpuDefault.Decimal,
		PpuOnOrder:                        req.PpuOnOrder.Decimal,
		TotalPackagePriceOnOrder:          req.TotalPackagePriceOnOrder.Decimal,
		PpuSoldPrice:                      req.PpuSoldPrice.Decimal,
		TotalSoldPrice:                    req.TotalSoldPrice.Decimal,
		PackagingSuppliesConsumed:         req.PackagingSuppliesConsumed.Bool,
		IsLineItem:                        req.IsLineItem.Bool,
		OrderID:                           req.OrderID,
		UomID:                             req.UomID.Int64,
